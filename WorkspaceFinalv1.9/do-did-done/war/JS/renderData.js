...
var getDBDate = function(someDate) {
	var result = someDate.getFullYear() + "-" + (someDate.getMonth() + 1) + "-" + someDate.getDate();
	return result;
}

var generateWeekDateHeader = function() {
	...
	for(var i = 0; i < 7 ; i++) {
		...
		td.setAttribute("style", "padding-bottom:8px; border-bottom: 1px solid #ddd;");
...
var createCalendarTable = function() {
	...
	calendarTable.appendChild(generateWeekDateHeader());
...
var formCalendar = function(dateNow) {
	...
	for(var i = 0; i < 7 ; i++) {
		var td = document.createElement('td');
		var varButton = document.createElement('button');
		varButton.setAttribute("id", getDBDate(endOfWeek));
		if(getDBDate(endOfWeek) === currentDate) {
			varButton.setAttribute("class", "w3-button w3-hover-purple w3-purple");
		} else {
